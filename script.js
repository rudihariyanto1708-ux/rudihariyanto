// script.js — handle product navigation & popup form (mobile-friendly)

// --- utility open product (katalog -> produk.html with params) ---
...
// --- POPUP FORM (in produk.html) ---
(function setupPopupHandlers(){
  const buyBtn = document.getElementById('buy-now');
  if(!buyBtn) return;

  // build popup HTML once and append to body
  const popupHtml = `
  <div class="popup" id="popup">
    <div class="popup-content">
      <span class="close-popup" id="closePopup">×</span>
      <h3 class="popup-title">🛍️ Formulir Pembelian</h3>
      <p class="popup-subtitle">Isi data untuk menyelesaikan pembelian</p>
      <form id="orderForm" class="order-form" onsubmit="return false;">
        ...
        <div class="floating-group"><input id="buyerPhone" placeholder=" " required><label>Nomor WhatsApp (contoh: 628123...)</label></div>
        ...
        <div class="popup-actions">
          <button class="confirm-btn" id="confirmBuy" type="button">Konfirmasi Beli 💬</button>
  ...
  confirmBuyBtn.addEventListener('click', function(){
    ...
    if(!name || !address || !phone){
      alert('Mohon isi Nama, Alamat, dan Nomor WhatsApp terlebih dahulu.');
      return;
    }

    // ganti WA_NUMBER di bawah dengan nomormu (atau biarkan user ganti di file)
    const WA_NUMBER = 'YOUR_WA_NUMBER'; // <-- ganti sendiri
    const message =
`Halo Sheila Store, saya ingin membeli:
- Produk: ${productName}
- Jumlah: ${qty}
